        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.finance.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.finances.update", [$finance->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('institutions') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="institutions" id="institutions" required>
                                ...
                                @foreach(App\Models\Finance::INSTITUTIONS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('institutions', $finance->institutions) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $finance->amount) }}" step="0.01" required>
                        ...
                        <div class="form-group {{ $errors->has('payment_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="payment_date" id="payment_date" value="{{ old('payment_date', $finance->payment_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="status" id="status" value="{{ old('status', $finance->status) }}" required>
